    ...
    @foreach (['ar' => __('Arabic'), 'en' => __('English')] as $locale => $language)
        <div class="row mb-3">
            <div class="col-md-6">
                ...
                <input type="text" id="name_{{ $locale }}" name="{{ $locale }}[name]" class="form-control" value="{{$product->name}}">
            </div>
            <div class="col-md-6">
                <label for="description_{{ $locale }}" class="form-label">{{ __('product description') }} {{ $language }}</label>
                <input type="text" id="description_{{ $locale }}" name="{{ $locale }}[description]" class="form-control" value="{{$product->description}}">
            </div>
        </div>
    @endforeach
{{-- 
